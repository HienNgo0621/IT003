...
int Delete(Hashtable &ht, int maso, int &nprob) {
    int M = ht.M;
    int index = maso % M;
    nprob = 0; // Bắt đầu thăm dò lần đầu

    for (int i = 0; i < M; i++) {
        int probeIndex = (index + i * i) % M;
        Hocsinh &hs = ht.table[probeIndex];

        if (hs.Maso == EMPTY) {
            // Gặp ô trống => chắc chắn không có phần tử này trong bảng
            return 0;
        }

        if (hs.Maso == maso) {
            hs.Maso = DELETE; // Đánh dấu đã xóa
            ht.n--;
            return 1;
        }
    ...
    // Không tìm thấy sau khi duyệt hết bảng
    return 0;
